...
.. class:: Parser
   ...
   .. function:: combine(fn)
      ...
      Example 2 - the argument order of our callable doesn't match, so
      we can fix it using a lambda.

      .. code:: python

         >>> ddmmyyyy = seq(regex(r'[0-9]{2}').map(int),
                            regex(r'[0-9]{2}').map(int),
                            regex(r'[0-9]{4}').map(int)).combine(lambda d, m, y: date(y, m, d))
         >>> ddmmyyyy.parse('06052014')
         datetime.date(2014, 5, 6)

      The equivalent ``lambda`` to use with ``map`` would be ``lambda res:
      date(res[2], res[1], res[0])``, which is less readable. The version with
      ``combine`` also ensures that exactly 3 items are generated by the previous
      parser, otherwise you get a ``TypeError``.
